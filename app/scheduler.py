...
#Короче тут я хуй подзабил, но оно вроде и не надо
def add_send_day_text_job(user_id, bot):
    job_id = f"send_day_text_{user_id}"
    if scheduler.get_job(job_id) is None:
        scheduler.add_job(send_day_text, trigger=IntervalTrigger(days=1, start_date='2024-09-13 15:17:00'), args=[user_id, bot], id=job_id)
